...
DROP CONSTRAINT IF EXISTS resource_files_stage_4c3_canonical_metadata_check;

ALTER TABLE public.resource_files
ADD CONSTRAINT resource_files_stage_4c4_canonical_metadata_check CHECK (
	(
	...
	OR (
		storage_key_version = 'generic_v2'::public.resource_storage_key_version
		AND file_kind = 'markdown'::public.resource_file_kind
		AND normalized_extension = '.md'
		AND content_type = 'text/plain'
		AND right(lower(display_filename), 3) = '.md'
	)
	OR (
		storage_key_version = 'generic_v2'::public.resource_storage_key_version
		AND file_kind = 'tex'::public.resource_file_kind
		AND normalized_extension = '.tex'
		AND content_type = 'text/plain'
		AND right(lower(display_filename), 4) = '.tex'
	)
	OR (
		storage_key_version = 'generic_v2'::public.resource_storage_key_version
		AND file_kind = 'text'::public.resource_file_kind
		AND normalized_extension = '.txt'
		AND content_type = 'text/plain'
		AND right(lower(display_filename), 4) = '.txt'
	)
	OR (
		storage_key_version = 'generic_v2'::public.resource_storage_key_version
		AND file_kind = 'source'::public.resource_file_kind
		AND normalized_extension IN (
			'.java', '.py', '.c', '.h', '.cpp', '.hpp',
			'.js', '.ts', '.rs', '.go', '.sql', '.sh'
		)
		AND content_type = 'text/plain'
		AND right(lower(display_filename), length(normalized_extension)) = normalized_extension
	)
);

ALTER TABLE public.resource_files
ADD CONSTRAINT resource_files_stage_4c4_text_size_check CHECK (
	file_kind NOT IN (
		'markdown'::public.resource_file_kind,
		'tex'::public.resource_file_kind,
		'text'::public.resource_file_kind,
		'source'::public.resource_file_kind
	)
	OR byte_size <= 2000000
);
...
BEGIN
	...
	IF requested_file_kind IN (
		'markdown'::public.resource_file_kind,
		'tex'::public.resource_file_kind,
		'text'::public.resource_file_kind,
		'source'::public.resource_file_kind
	) AND byte_size > 2000000 THEN
		RAISE EXCEPTION 'text file size must be between 1 and 2000000 bytes'
			USING ERRCODE = '23514';
	END IF;
	...
	IF NOT (
		...
		OR (
			requested_file_kind IS NOT DISTINCT FROM 'markdown'::public.resource_file_kind
			AND requested_normalized_extension IS NOT DISTINCT FROM '.md'
			AND requested_content_type IS NOT DISTINCT FROM 'text/plain'
			AND right(lower(normalized_display_filename), 3) = '.md'
		)
		OR (
			requested_file_kind IS NOT DISTINCT FROM 'tex'::public.resource_file_kind
			AND requested_normalized_extension IS NOT DISTINCT FROM '.tex'
			AND requested_content_type IS NOT DISTINCT FROM 'text/plain'
			AND right(lower(normalized_display_filename), 4) = '.tex'
		)
		OR (
			requested_file_kind IS NOT DISTINCT FROM 'text'::public.resource_file_kind
			AND requested_normalized_extension IS NOT DISTINCT FROM '.txt'
			AND requested_content_type IS NOT DISTINCT FROM 'text/plain'
			AND right(lower(normalized_display_filename), 4) = '.txt'
		)
		OR (
			requested_file_kind IS NOT DISTINCT FROM 'source'::public.resource_file_kind
			AND requested_normalized_extension IN (
				'.java', '.py', '.c', '.h', '.cpp', '.hpp',
				'.js', '.ts', '.rs', '.go', '.sql', '.sh'
			)
			AND requested_content_type IS NOT DISTINCT FROM 'text/plain'
			AND right(
				lower(normalized_display_filename),
				length(requested_normalized_extension)
