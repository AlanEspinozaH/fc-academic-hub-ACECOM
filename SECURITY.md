...
La etapa 3B.1 mantiene la aplicacion Astro sin login visible, OAuth, paginas privadas ni administracion de roles. Agrega reglas PostgreSQL reproducibles para validar dominios institucionales en `auth.users` y sincronizar `public.profiles` sobre los fundamentos locales de roles, auditoria y RLS. No conecta Cloudflare R2 ni proveedores OAuth.
